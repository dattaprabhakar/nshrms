...
try:
    client = MongoClient(os.getenv("MONGO_URI"))
    # Explicitly get the database named 'helpdesk_db' or your chosen name
    db_name = os.getenv("MONGO_URI").split('/')[-1].split('?')[0] # Attempt to get DB name from URI
    db = client[db_name]
    users_collection = db.users
    attendance_collection = db.attendance
    leaves_collection = db.leaves
...
def dashboard():
    ...
    # Pass 'now' context for templates (handled by context_processor now)
    return render_template('dashboard.html', user=user, attendance_status=attendance_status)
...
def view_payslips():
    ...
    try:
        ...
        # Example: Add dummy data if empty for testing (REMOVE IN PRODUCTION)
        # Consider doing this check only once or via an admin function
        if not user_payslips and payslips_collection.count_documents({'user_id': user['_id']}) == 0:
             payslips_collection.insert_many([
                 {'user_id': user['_id'], 'month': 1, 'year': 2024, 'file_name': 'payslip_jan_2024.pdf', 'net_pay': 5100.75, 'generated_date': datetime(2024, 1, 31)},
                 {'user_id': user['_id'], 'month': 12, 'year': 2023, 'file_name': 'payslip_dec_2023.pdf', 'net_pay': 5050.50, 'generated_date': datetime(2023, 12, 31)}
             ])
             user_payslips = list(payslips_collection.find(
                 {'user_id': user['_id']}
             ).sort([('year', -1), ('month', -1)]))
...
if __name__ == '__main__':
    ...
    print(f"--> Access locally: http://127.0.0.1:5001")
    print(f"--> Access on your network: http://<YOUR_MACHINE_IP>:5001")
    print("         (Replace <YOUR_MACHINE_IP> with your actual IP address)")
    print("--- Press CTRL+C to quit ---")
    app.run(host='0.0.0.0', port=5001, debug=True) # <<< MODIFIED LINE